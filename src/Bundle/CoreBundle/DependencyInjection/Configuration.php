                ...
                ->arrayNode('classes')
                    ...
                    ->children()
                        ...
                        ->arrayNode('import_patient')
                            ...
                            ->end()
                        ->end()
                        ->arrayNode('import_activity')
                            ->addDefaultsIfNotSet()
                            ->children()
                                ->scalarNode('model')->defaultValue('Accard\Component\Core\Model\ImportActivity')->end()
                                ->scalarNode('controller')->defaultValue('Accard\Bundle\ResourceBundle\Controller\ResourceController')->end()
                                ->scalarNode('repository')->defaultValue('Accard\Bundle\CoreBundle\Doctrine\ORM\ImportActivityRepository')->end()
                                ->scalarNode('form')->defaultValue('Accard\Bundle\CoreBundle\Form\Type\ImportActivityType')->end()
                            ->end()
                        ->end()
                        ->arrayNode('import_sample')
                            ->addDefaultsIfNotSet()
                            ->children()
                                ->scalarNode('model')->defaultValue('Accard\Component\Core\Model\ImportSample')->end()
                                ->scalarNode('controller')->defaultValue('Accard\Bundle\ResourceBundle\Controller\ResourceController')->end()
                                ->scalarNode('repository')->defaultValue('Accard\Bundle\CoreBundle\Doctrine\ORM\ImportSampleRepository')->end()
                                ->scalarNode('form')->defaultValue('Accard\Bundle\CoreBundle\Form\Type\ImportSampleType')->end()
                            ->end()
                        ->end()
                    ->end()
                ->end()
